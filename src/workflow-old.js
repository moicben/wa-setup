...
const { StrictDecisionEngine } = require('./core/decision/StrictDecisionEngine');
...
class WhatsAppWorkflow {
    ...
    async createAccount(country = null, maxRetries = 3) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // Vérifier le succès de l'achat AVANT de sauvegarder
                if (!numberResult.success) {
                    throw new Error(`Impossible d'acheter un numéro: ${numberResult.error}`);
                }
                
                // Valider les propriétés requises
                if (!numberResult.number || !numberResult.id) {
                    throw new Error('Numéro SMS acheté mais données incomplètes');
                }
                
                // Sauvegarder en base de données SEULEMENT après succès
                if (this.accountRepo) {
                    try {
                ...
                if (!phoneSMSCheck.canReceiveSMS) {
                    ...
                    // SAUVETAGE-PHASE-1: Décision stricte avec StrictDecisionEngine
                    ...
                    // Traitement selon la décision STRICTE
                    if (decision.decision === 'CONTINUE') {
                        ...
                        // Pour l'instant, on traite VALIDATE comme ABORT pour être strict
                        console.error('🚨 VALIDATION SUPPLÉMENTAIRE REQUISE = ABANDON IMMÉDIAT');
                        
                        // Mise à jour du statut en base de données
                        if (this.accountRepo && this.session?.phone) {
                            try {
                                await this.accountRepo.updateAccountStatus(this.session.phone, 'error', {
                                    error: 'SMS_VALIDATION_REQUIRED_ABORT',
                                    reason: 'Validation supplémentaire requise',
                                    attempt: attempt,
                                    failed_at: new Date().toISOString()
                                });
                                console.log('📊 Statut mis à jour: error (validation)');
                            } catch (dbError) {
                                console.warn('⚠️ Erreur mise à jour statut:', dbError.message);
                            }
                        }
                        ...
                        if (this.accountRepo && this.session?.phone) {
                            try {
                                await this.accountRepo.updateAccountStatus(this.session.phone, 'error', {
                                    error: 'SMS_CONFIDENCE_TOO_LOW_RETRY_NEEDED',
                                    reason: decision.reason,
                                    confidence: decision.confidence,
                                    attempt: attempt,
                                    failed_at: new Date().toISOString()
                                });
                                console.log('📊 Statut mis à jour: error (confiance faible)');
                            } catch (dbError) {
                                console.warn('⚠️ Erreur mise à jour statut:', dbError.message);
                            }
                        }
                ...
                if (postSMSAnalysis.isSMSFailure) {
                    ...
                    // SAUVETAGE-PHASE-1: Abandon immédiat si analyse post-SMS échoue
                    ...
                    if (this.accountRepo && this.session?.phone) {
                        try {
                            await this.accountRepo.updateAccountStatus(this.session.phone, 'error', {
                                error: 'POST_SMS_ANALYSIS_FAILED_ABORT',
                                reason: 'Analyse post-SMS échouée',
                                attempt: attempt,
                                failed_at: new Date().toISOString()
                            });
                            console.log('📊 Statut mis à jour: error (analyse post-SMS)');
                        } catch (dbError) {
                            console.warn('⚠️ Erreur mise à jour statut:', dbError.message);
                        }
                    }
                ...
                if (this.metrics) {
                    try {
                        await this.metrics.recordWorkflowMetric({
                ...
                // Mise à jour du statut de succès en base de données
                if (this.accountRepo && this.session?.phone) {
                    try {
                        await this.accountRepo.updateAccountStatus(this.session.phone, 'success', {
                            completed_at: new Date().toISOString(),
                            duration_seconds: duration,
                            final_attempt: attempt,
                            full_number: numberResult.fullNumber
                        });
                        console.log('📊 Statut mis à jour: success');
                    } catch (dbError) {
                        console.warn('⚠️ Erreur mise à jour statut succès:', dbError.message);
                    }
                }
                ...
                if (this.accountRepo && this.session?.phone) {
                    try {
                        await this.accountRepo.updateAccountStatus(this.session.phone, 'failed', {
                            error: error.message,
                            attempt: attempt,
                            failed_at: new Date().toISOString()
                        });
                ...
                if (attempt === maxRetries) {
                    ...
                    // Mise à jour du statut final en base de données
                    if (this.accountRepo && this.session?.phone) {
                        try {
                            await this.accountRepo.updateAccountStatus(this.session.phone, 'failed_final', {
                                error: error.message,
                                total_attempts: maxRetries,
                                final_failure_at: new Date().toISOString()
                            });
                            console.log('📊 Statut mis à jour: failed_final');
                        } catch (dbError) {
                            console.warn('⚠️ Erreur mise à jour statut final:', dbError.message);
                        }
                    }
                    return {
                        success: false,
    ...
    analyzeExtractedText(text) {
        ...
        if (isVerificationScreen && (isWaitingForSMS || hasSMSCodeMessage || hasCodeSentMessage)) {
            ...
            // SAUVETAGE-PHASE-1: Validation renforcée avec seuil élevé
            ...
            // SAUVETAGE-PHASE-1: Validation stricte des méthodes
    ...
    detectSMSFailure(text) {
        ...
        // SAUVETAGE-PHASE-1: Seuil strict pour détection d'échec SMS
    ...
    /**
     * SAUVETAGE-PHASE-1: Validation stricte de l'interface continue+verify
     * 
     * @param {string} text - Texte extrait par OCR
     * @returns {Object} Résultat de validation stricte
     */
    performStrictInterfaceValidation(text) {
    ...
    /**
     * SAUVETAGE-PHASE-1: Validation stricte des méthodes de vérification
     * 
