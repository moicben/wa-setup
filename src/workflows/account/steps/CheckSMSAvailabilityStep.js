...
const { OCRAnalyzer } = require('../../utils/OCRAnalyzer');
...
class CheckSMSAvailabilityStep extends BaseStep {
    ...
    async _analyzeVerificationOptions(context, screenshotPath) {
        ...
        // Initialiser l'analyseur OCR
        await this.ocrAnalyzer.initialize();
        
        // Analyser le screenshot
        const ocrResult = await this.ocrAnalyzer.analyzeVerificationOptions(screenshotPath);
    ...
    async canExecute(context) {
        try {
            ...
            // Initialiser l'analyseur OCR
            const ocrInitialized = await this.ocrAnalyzer.initialize();
            if (!ocrInitialized) {
